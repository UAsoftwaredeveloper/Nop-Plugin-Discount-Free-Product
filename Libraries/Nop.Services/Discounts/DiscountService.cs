...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        #region Methods
        #region DiscountMapping
        /// <summary>
        /// Get All Discount Category Mapping
        /// </summary>      
        /// <returns>A task that represents the asynchronous operation</returns>

        public virtual async Task<IList<DiscountCategoryMapping>> GetAllDiscountCatgoryMappingsAsync()
        {
            IList<DiscountCategoryMapping> discountCategoryMappings = await _discountCategoryMappingRepository.Table.ToListAsync();
            return discountCategoryMappings;
        }
        /// <summary>
        /// Get All Discount Mappings
        /// </summary>
        /// <returns>A task that represents the asynchronous operation</returns>

        public virtual async Task<IList<DiscountMapping>> GetAllDiscountMappingsAsync()
        {
            return await _discountMappingRepository.Table.ToListAsync();

        }
        /// <summary>
        /// Get EntityId By DiscountId 
        /// Created By Umesh Aggarwal Software Developer
        /// </summary>
        /// <param name="discountId">EntityId By DiscountId</param>
        /// <returns>A task that represents the asynchronous operation</returns>

        public virtual async Task<(int, string)> GetEntityIdByDiscountIdAsync(int discountId)
        {
            int EntityId = 0;
            string entityname = string.Empty;
            if (EntityId == 0)
            {
                List<DiscountCategoryMapping> discountCategoryMappings = await _discountCategoryMappingRepository.Table.Where(x => x.DiscountId == discountId).ToListAsync();
                if (discountCategoryMappings.Count > 0)
                    EntityId = discountCategoryMappings.Where(x => x.DiscountId == discountId).FirstOrDefault().EntityId;
                entityname = "Category";
            }
            if (EntityId == 0)
            {
                List<DiscountManufacturerMapping> discountManufacturerMappings = await _discountManufacturerMappingRepository.Table.Where(x => x.DiscountId == discountId).ToListAsync();
                if (discountManufacturerMappings.Count > 0)
                    EntityId = discountManufacturerMappings.Where(x => x.DiscountId == discountId).FirstOrDefault().EntityId;
                entityname = "Manufacturer";

            }
            if (EntityId == 0)
            {
                List<DiscountProductMapping> discountProductMappings = await _discountProductMappingRepository.Table.Where(x => x.DiscountId == discountId).ToListAsync();
                if (discountProductMappings.Count > 0)
                    EntityId = discountProductMappings.Where(x => x.DiscountId == discountId).FirstOrDefault().EntityId;
                entityname = "Product";
