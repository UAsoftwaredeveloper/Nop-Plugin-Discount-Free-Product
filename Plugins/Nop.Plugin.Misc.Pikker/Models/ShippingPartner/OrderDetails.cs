...
namespace Nop.Web.Areas.Admin.Models.ShippingPartner
{
    ...
    {
        string token = "e2495ab1f1aa403c0fcb12e9e7fffed9135608";
        public string get_dataPickerForDispatch(OrderModel order, IGenericAttributeService _genericAttributeService, Customer customer, IProductService _productService)
        {
            ...
            { // call payment //allready set COd as C and prepaid is p
                ...
                consignments.from_name = order.PickupAddress.Company;
                consignments.from_phone_number =order.PickupAddress.PhoneNumber;
                consignments.from_address = order.PickupAddress.Address1+","+order.PickupAddress.City;
                consignments.from_pincode = order.PickupAddress.PhoneNumber ;
        ...
        public string PrepareOrderCancellation(string trackingId)
        {
            try
            {
                ...
                consignments = new JObject();
                consignments.auth_token = token;
                consignments.item_name = trackingId;
                string json = JsonConvert.SerializeObject(consignments, Newtonsoft.Json.Formatting.Indented);
                return json.ToString();
            }
